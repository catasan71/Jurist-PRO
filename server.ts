...
app.post(['/api/gemini', '/gemini'], async (req, res) => {
  ...
  try {
    ...
    // Request stream from Gemini with custom configs and resilient model fallback + retry
    const candidateModels = ['gemini-3-flash-preview', 'gemini-2.5-flash', 'gemini-2.0-flash'];
    let stream: any = null;
    ...
    for (const modelName of candidateModels) {
      ...
      while (retries >= 0) {
        try {
          console.log(`[GEMINI] Attempting generation with model ${modelName} (remaining retries: ${retries})...`);
          ...
          break; // Succeeded in initiating stream
        } catch (streamError: any) {
          lastError = streamError;
          const msg = (streamError.message || '').toLowerCase();
          const isQuotaOrRateLimit = msg.includes('resource_exhausted') || msg.includes('429') || msg.includes('quota') || msg.includes('too many requests') || msg.includes('overloaded') || msg.includes('503');
          
          if (isQuotaOrRateLimit && retries > 0) {
            console.warn(`[GEMINI] Temporary rate limit/overload on ${modelName}. Waiting 1.5s before retry...`);
            await new Promise(resolve => setTimeout(resolve, 1500));
            retries--;
            continue;
          }

          // Try without tools if tools/grounding caused issue
          try {
            console.log(`[GEMINI] Trying ${modelName} without search tools...`);
            stream = await ai.models.generateContentStream({
              model: modelName,
              contents,
              config: {
            ...
            console.warn(`[GEMINI] Failed ${modelName} fallback without tools:`, noToolsError.message);
    ...
    if (!stream) {
      throw lastError || new Error('Nu s-a putut inițializa generarea AI.');
    }
    ...
    } else if (errMsg.includes('RESOURCE_EXHAUSTED') || errMsg.includes('429') || errMsg.includes('Quota exceeded') || errMsg.includes('quota')) {
        errMsg = 'Limita de cereri/minut a cheii API (Google Free Tier Quota) a fost temporar atinsă. Vă rugăm să reîncercați în 30-60 de secunde.';
    }
