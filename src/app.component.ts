...
@Component({
  ...
  template: `
    ...
    <div class="fixed top-4 right-4 z-[100] flex flex-col gap-3 pointer-events-none max-w-lg">
      @for (n of notificationService.notifications(); track n.id) {
        <div class="pointer-events-auto animate-slideDown shadow-2xl rounded-xl border border-white/10 overflow-hidden flex items-start min-w-[320px] max-w-lg backdrop-blur-md"
             [class.bg-emerald-600]="n.type === 'success'"
             [class.bg-red-700]="n.type === 'error'"
             [class.bg-amber-600]="n.type === 'warning'"
             [class.bg-blue-600]="n.type === 'info'">
          <div class="p-4 text-white flex-1 font-medium text-sm leading-relaxed whitespace-pre-line">
            {{ n.message }}
          </div>
          <button (click)="notificationService.remove(n.id)" class="p-4 text-white/80 hover:text-white hover:bg-black/20 transition-colors flex-shrink-0" title="Închide">
