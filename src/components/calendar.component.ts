    ...
    <div class="h-full flex flex-col bg-jurist-card rounded-xl border border-gray-800 shadow-neon overflow-hidden relative animate-fadeIn">
      ...
      <div class="flex-1 overflow-y-auto p-4 lg:p-6 flex flex-col lg:flex-row gap-6 animate-slideUp">
        ...
        <div [class]="'flex-1 space-y-6 ' + (mobileTab() === 'agenda' ? 'block' : 'hidden lg:block')">
          ...
          <!-- WHATSAPP PENDING ALERTS BANNER -->
          @if (juristService.readyAlerts().length > 0) {
            <div class="bg-green-900/40 border border-green-500/50 rounded-xl p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 animate-fadeIn">
              <div class="flex items-center gap-3">
                <div class="p-2 bg-green-500 rounded-full animate-pulse shadow-[0_0_15px_rgba(34,197,94,0.5)]">
                ...
                <div>
                  <h4 class="text-green-400 font-bold text-sm">Aveți {{ juristService.readyAlerts().length }} alertă(e) WhatsApp de trimis!</h4>
                  <p class="text-[10px] text-green-200">Termene programate mâine pentru care beneficiarii trebuie notificați.</p>
                </div>
              </div>
              <button (click)="sendAllPendingAlerts()" class="w-full sm:w-auto px-4 py-2 bg-green-500 hover:bg-green-600 text-black font-bold text-xs rounded-lg transition-colors whitespace-nowrap shadow-lg flex items-center justify-center gap-2">
                TRIMITE TOATE ALERTELE
              </button>
            </div>
          }
